...
packages <- c("multiScaleR", "terra", "sf", "readxl", "MASS",
              "ggplot2", "dplyr", "tmap")
...
# All maps in this tutorial are interactive
tmap_mode("view")

# Verify multiScaleR version
if (packageVersion("multiScaleR") < "0.6.13") {
  warning("multiScaleR >= 0.6.13 is required for this tutorial. Update with install.packages('multiScaleR')")
}
...
#| fig-cap: "PCA urbanization raster for the Cleveland Metroparks study area. Warm colors indicate high urbanization; cool colors indicate low urbanization (more forested). Points show camera trap locations."

tm_shape(urb_pca) +
  tm_raster(palette = "-RdYlBu", title = "PCA Urbanization") +
  tm_shape(cam_sf) +
  tm_dots(col = "black", size = 0.05, alpha = 0.8, title = "Camera sites")
```
...
```{r}
#| label: fig-nlcd
#| fig-cap: "Binary land cover rasters derived from the 2016 NLCD. Left: developed land (classes 21-24). Right: forested land (classes 41-43). Maps are synchronized — panning or zooming one updates the other."

m_dev <- tm_shape(developed) +
  tm_raster(palette = c("lightyellow", "firebrick"), style = "cat",
            title = "Developed", labels = c("No", "Yes")) +
  tm_shape(cam_sf) + tm_dots(size = 0.03, alpha = 0.7)

m_for <- tm_shape(forested) +
  tm_raster(palette = c("lightyellow", "darkgreen"), style = "cat",
            title = "Forested", labels = c("No", "Yes")) +
  tm_shape(cam_sf) + tm_dots(size = 0.03, alpha = 0.7)

tmap_arrange(m_dev, m_for, ncol = 2, sync = TRUE)
```
...
- **Development** (sigma = 30 m, boundary): no significant effect on deer counts (p = 0.19). There is no detectable relationship between local development and deer counts at any scale we can estimate.
- **Forest cover** (sigma approximately 1524 m): strong negative effect (beta = -0.43, p \< 0.001). Deer counts are lower at sites surrounded by more forest at a landscape scale.

The PCA composite showed a positive effect of urbanization on deer (beta = +0.36, p \< 0.001). The NLCD decomposition clarifies what is actually driving that signal: it is landscape-scale forest composition, not local development. The two components of the PCA urbanization axis operate at different scales and have different relationships with deer abundance, and lumping them into a single composite index hides that distinction.

This is a key practical lesson: composite covariates can produce interpretable results while obscuring the underlying ecological mechanism. Decomposing them into constituent parts, and allowing each part its own optimized scale, can substantially sharpen inference.
:::
...
tm_shape(pred_cat) +
  tm_raster(palette = "YlOrRd", title = "Predicted counts") +
  tm_shape(cam_sf) +
  tm_dots(col = "black", size = 0.04, alpha = 0.7, title = "Camera sites")
```
...
tm_shape(pred_cat) +
  tm_raster(palette = "YlOrRd", title = "Predicted counts") +
  tm_shape(cam_sf) +
  tm_dots(col = "black", size = 0.04, alpha = 0.7)
```
...
tm_shape(pred_deer) +
  tm_raster(palette = "YlGn", title = "Predicted counts") +
  tm_shape(cam_sf) +
  tm_dots(col = "black", size = 0.04, alpha = 0.7)
```
...
tm_shape(pred_deer_nlcd) +
  tm_raster(palette = "PuBuGn", title = "Predicted counts") +
  tm_shape(cam_sf) +
  tm_dots(col = "black", size = 0.04, alpha = 0.7)
```
:::
...
3.  **Composite covariates can obscure ecological mechanisms.** The PCA urbanization index showed a positive effect of urbanization on deer, but the NLCD decomposition revealed that the two components of that index operate at different scales and in different ways. Landscape-scale forest composition drives the pattern; local development does not. A composite that blends these components into one axis would never reveal this distinction.
